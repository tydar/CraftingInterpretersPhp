...
{ 
    ...
    defineVisitorTrait($outputDir, $baseName, $types);
}
...
function defineVisitorTrait(string $outputDir, string $baseName, array $types) : void
{
    ...
    $contents .= '// Generated by generate_ast.php on ' . date('d/M/Y') . PHP_EOL . PHP_EOL;
    $contents .= "namespace Lox;" . PHP_EOL;
    $contents .= "trait ExprVisitor {" . PHP_EOL;
    
    foreach($types as $type) {
        $type = trim($type);
        $className = trim(explode(":", $type)[0]);
        $contents .= '    abstract function visit' . $className . $baseName . "(" . $className . ' $' . strtolower($baseName) . ') : mixed;' . PHP_EOL;
    }
